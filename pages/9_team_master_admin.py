...
import sys
sys.path.append("..")
from db import get_connection, get_teams, hide_default_sidebar_navigation
...
with tab1:
    ...
    if not teams_df.empty:
        ...
        def color_preview(color):
            return f'<div style="width: 50px; height: 20px; background-color: {color}; border: 1px solid #ccc;"></div>'
        ...
        for _, row in display_df.iterrows():
            ...
            with col2:
                st.markdown(color_preview(row['color']), unsafe_allow_html=True)
...
with tab2:
    ...
    with st.form("add_team_form"):
        ...
        with preview_col1:
            st.markdown(f'<div style="width: 100px; height: 50px; background-color: {color}; border: 1px solid #ccc; border-radius: 4px;"></div>', unsafe_allow_html=True)
        with preview_col2:
...
with tab3:
    ...
    if not teams_df.empty:
        ...
        if selected_team_name:
            ...
            team_data = teams_df[teams_df["team_id"] == selected_team_id].iloc[0]
            ...
            with col1:
                ...
                with st.form("edit_team_form"):
                    ...
                    preview_col1, preview_col2 = st.columns([1, 3])
                    with preview_col1:
                        st.markdown(f'<div style="width: 100px; height: 50px; background-color: {new_color}; border: 1px solid #ccc; border-radius: 4px;"></div>', unsafe_allow_html=True)
                    with preview_col2:
                    ...
                    if update:
                        if not new_short_name.strip():
                            ...
                            try:
                                ...
                                if cursor.fetchone():
                                    st.error(f"❌ 略称 '{new_short_name}' は既に使用されています")
                                else:
                                    # チーム情報を更新
                                    cursor.execute("""
                                        UPDATE teams 
                                    ...
                                    conn.commit()
                                    st.success(f"✅ {new_short_name} の情報を更新しました")
                                    st.rerun()
