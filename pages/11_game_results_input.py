...
with tab_new:
    ...
    if last_game:
        default_season_idx = seasons.index(
            last_game[0]) if last_game[0] in seasons else 0
        default_date = datetime.strptime(last_game[1], "%Y-%m-%d").date()
        default_table_type_idx = ["レギュラー", "セミファイナル", "ファイナル", "その他"].index(
            last_game[2]) if last_game[2] in ["レギュラー", "セミファイナル", "ファイナル", "その他"] else 0
    ...
    with col1:
        selected_season = st.selectbox(
            "シーズン", seasons, index=default_season_idx, key="new_season_select")
    ...
    with col3:
        ...
        table_type = st.selectbox(
            "卓区分", table_types, index=default_table_type_idx, key="new_table_type")
    ...
    # チームリスト（表示用）
    team_list = [(team_id, teams_dict[team_id])
                 for team_id in sorted(teams_dict.keys())]
    ...
    for i, seat in enumerate(seat_names):
        ...
        # 選手選択用の閉包を作成（ラムダ関数が正しい team_player_list を参照するため）
        def make_player_formatter(player_list):
            return lambda x: player_list[x][1]
        ...
        with cols[2]:
            ...
            player_idx = st.selectbox(
                ...
                format_func=make_player_formatter(team_player_list),
    ...
    recent_df = pd.read_sql_query(
        recent_games_query, conn, params=(selected_season,))
    conn.close()
...
with tab_edit:
    ...
    with col1:
        edit_season = st.selectbox(
            "シーズン", edit_seasons, key="edit_season_select")
