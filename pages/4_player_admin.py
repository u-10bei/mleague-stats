...
with tab1:
    ...
    if not players_df.empty:
        ...
        # カラム名を変更
        display_df.columns = ["player_id", "選手名",
                              "生年月日", "所属団体", "所属チーム", "season"]
        ...
        with col2:
            org_filter = st.multiselect(
                "所属団体でフィルター",
                options=sorted(
                    [x for x in display_df["所属団体"].unique() if x != "-"]),
                default=None
            )
        ...
        with col2:
            with_birthdate = len(
                [x for x in players_df["birth_date"] if pd.notna(x)])
...
with tab2:
    ...
    if current_season is None:
        ...
        team_options = dict(
            zip(team_names_df["team_name"], team_names_df["team_id"]))
        ...
        with st.form("add_player_form"):
            ...
            selected_team_name = st.selectbox(
                "所属チーム", options=list(team_options.keys()))
...
with tab3:
    ...
    if not players_df.empty:
        ...
        player_display_df["display_name"] = player_display_df["player_name"] + \
            " (" + player_display_df["team_name"] + ")"
        player_options = dict(
            zip(player_display_df["display_name"], player_display_df["player_id"]))
        ...
        if selected_player_display:
            ...
            player_data = player_display_df[player_display_df["player_id"]
                                            == selected_player_id].iloc[0]
            ...
            with col1:
                ...
                with st.form("edit_player_info_form"):
                    ...
                    with edit_col1:
                        ...
                        if pd.notna(player_data["birth_date"]) and player_data["birth_date"]:
                            try:
                                current_birth_date = pd.to_datetime(
                    ...
                    with edit_col2:
                        ...
                        st.caption(
                            "💡 主な団体: 日本プロ麻雀協会、日本プロ麻雀連盟、最高位戦日本プロ麻雀協会、RMU、麻将連合")

                    update_info = st.form_submit_button(
                        "💾 基本情報を更新", type="primary")
                    ...
                    if update_info:
                        if not new_player_name.strip():
                            ...
                            try:
                                ...
                                if cursor.fetchone():
                                    st.error(
                                        f"❌ {new_player_name} は既に登録されています")
                                else:
                                    # birth_dateとpro_orgの処理
                                    new_birth_date_str = new_birth_date.strftime(
                                        "%Y-%m-%d") if new_birth_date else None
                ...
                if current_season:
                    ...
                    with st.form("edit_player_team_form"):
                        ...
                        update_team = st.form_submit_button(
                            "💾 チーム所属を更新", type="primary")
                        ...
                        if update_team:
                            try:
                                ...
                                st.success(
                                    f"✅ {player_data['player_name']} のチーム所属を更新しました")
                                st.rerun()
                            except (ValueError, TypeError, RuntimeError) as e:
                                st.error(f"❌ エラーが発生しました: {e}")
            ...
            with col2:
                ...
                if st.button("削除", type="secondary"):
                    try:
                        ...
                        if stats_count > 0:
                            st.error(
                                f"❌ この選手には{stats_count}件の成績データがあります。先に成績データを削除してください。")
                        else:
                            ...
                            conn.commit()
                            st.success(
                                f"✅ {player_data['player_name']} を削除しました")
                            st.rerun()
